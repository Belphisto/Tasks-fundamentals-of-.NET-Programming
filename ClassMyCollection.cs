...
namespace Space
{
    ...
    public class MyCollection<T> : ICollection<T> where T : IPlanet 
    {
        ...
        public async Task AsyncSortByField(Func<IPlanet, IPlanet, bool> func)
        {
            await Task.Delay(5000);
            Log.WriteLog("Start AsyncSort Planet");
            await Task.Run(() => SortByField(func));
            Log.WriteLog("End AsyncSort Planet");
        }

        public void SortByField(Func<IPlanet, IPlanet, bool> func)
        {

            Log.WriteLog("Sort Planet by Field START");
            for (int i=0; i<innerPlanet.Count; i++)
            {
                ...
                Log.WriteLog($"Planet {i} sorted");
                Thread.Sleep(1000);
            }
            Log.WriteLog("Sort Planet by Field END");
        }


        public async Task AsyncSortByField(Func<ISpaceObject, ISpaceObject, bool> func)
        {
            await Task.Delay(3000);
            Log.WriteLog("Start AsyncSort SpaceObject");
            await Task.Run(() => SortByField(func));
            Log.WriteLog("End AsyncSort SpaceObject");
        }
        public void SortByField(Func<ISpaceObject, ISpaceObject, bool> func)
        {
            Log.WriteLog("Sort SpaceObject by Field START");
            for (int i=0; i<innerSpace.Count; i++)
            {
                for(int j=i+1; j<innerSpace.Count; j++)
                {
                    if(func(innerSpace[i], innerSpace[j]))
                    {
                        (innerSpace[i], innerSpace[j]) = (innerSpace[j], innerSpace[i]);
                    }
                }
                Log.WriteLog($"SpaceObject {i} sorted");
                Thread.Sleep(500);
            }
            Log.WriteLog("Sort SpaceObject by Field END");
        }
